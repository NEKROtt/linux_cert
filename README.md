# Операционные системы и виртуализация (Linux). Аттестация
...
cat > delfiletodir
```
Далее прописываем нижеуказанный текст:
```
#!/bin/bash
...
Первым делом, считываем всех владельцев файлов в директории, убираем повторения и делаем папки по одной для каждого владельца:
```
#!/bin/bash
...
```
Теперь считываем владельцев и названия. Проходимся по ним циклом (чётное - название, нечётное - владелец)
